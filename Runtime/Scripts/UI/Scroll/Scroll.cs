...
namespace ENP.UnityExtensions.Runtime.Scripts.UI.Scroll
{
        ...
        IInitializePotentialDragHandler, IBeginDragHandler, IDragHandler, IEndDragHandler, IScrollHandler
    {
        ...
        private readonly Subject<int> _snapTargetIndexSubject = new Subject<int>();
        ...
        public IObservable<int> SnapTargetIndexChanged => _snapTargetIndexSubject;
        ...
        private int _lastNotifiedSnapTargetIndex = -1;
        ...
        private void OnDestroy()
        {
            ...
            CompleteAndDispose(_snapTargetIndexSubject);
        ...
        private void EndDragSelf()
        {
            ...
            if (_snapEnabled)
            {
                ...
                _snapping = false;
                _snapDampVel = 0f;
                StartSnap(GetAxisValue(_velocity));
            }
        }
        ...
        private void StartSnap(float axisVelocity)
        {
            ...
            int targetPageIndex = _snapPageBySorted[chosenSorted];
            NotifySnapTargetChanged(targetPageIndex);
        ...
        private void EnsureSnapTargets()
        {
            ...
            if (_snapPageOrder == SnapPageOrder.Hierarchy)
            {
                for (int i = 0; i < childCount; i++)
                {
                    ...
                    Bounds b = GetRectBoundsInViewSpace(child);
        ...
        private void NotifySnapTargetChanged(int pageIndex)
        {
            if (pageIndex < 0 || pageIndex == _lastNotifiedSnapTargetIndex)
                return;

            _lastNotifiedSnapTargetIndex = pageIndex;
            _snapTargetIndexSubject.OnNext(pageIndex);
        }

        private void NotifySnapTargetByAxis(float axis)
        {
            EnsureSnapTargets();
            if (_snapTargetsCount <= 0)
                return;
            ...
            NotifySnapTargetChanged(_snapPageBySorted[nearestSorted]);
        }

        private Bounds GetRectBoundsInViewSpace(RectTransform rectTransform)
        {
            Vector3[] corners = new Vector3[4];
            rectTransform.GetWorldCorners(corners);

            Vector3 min = _viewRect.InverseTransformPoint(corners[0]);
            Vector3 max = min;

            for (int i = 1; i < 4; i++)
            {
                Vector3 corner = _viewRect.InverseTransformPoint(corners[i]);
        ...
        public void SetNormalizedPosition(float normalized, bool immediate = true)
        {
            ...
            SetContentAnchoredPosition(pos);
            if (_snapEnabled)
                NotifySnapTargetByAxis(GetAxisValue(pos));
        }
        ...
        public void SetPage(int pageIndex, bool immediate = true)
        {
            ...
            if (!immediate)
            {
                ...
                _pendingSnap = false;
                _hasPrev = false;
                NotifySnapTargetChanged(pageIndex);
                return;
            }
